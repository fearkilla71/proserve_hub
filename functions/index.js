...
// Learning loop: update the accepted quote training record with actual escrow outcome.
exports.onJobEscrowReleasedUpdateTraining = functions.firestore
  .document('job_requests/{jobId}')
  .onUpdate(async (change, context) => {
    const before = change.before.data() || {};
    const after = change.after.data() || {};

    const beforeEscrow = (before.escrowStatus || '').toString().trim().toLowerCase();
    const afterEscrow = (after.escrowStatus || '').toString().trim().toLowerCase();
    if (beforeEscrow === 'released') return null;
    if (afterEscrow !== 'released') return null;

    const acceptedQuoteId = (after.acceptedQuoteId || '').toString().trim();
    if (!acceptedQuoteId) return null;

    try {
      const db = admin.firestore();

      // Use the same 7.5% fee rule as createEscrowPayment for consistency.
      const amountCents = dollarsToCents(after.price);
      const feeCents = amountCents ? Math.round(amountCents * 0.075) : 0;
      const payoutCents = amountCents ? Math.max(amountCents - feeCents, 0) : 0;

      const trainingRef = db.collection('ai_quote_training').doc(acceptedQuoteId);
      await trainingRef.set(
        {
          escrowOutcome: {
            jobId: context.params.jobId,
            escrowStatus: afterEscrow,
            paymentIntentId: (after.paymentIntentId || '').toString(),
            jobTotal: amountCents ? amountCents / 100 : null,
            platformFeeAmount: feeCents ? feeCents / 100 : null,
            contractorPayout: payoutCents ? payoutCents / 100 : null,
            escrowReleasedAt: after.escrowReleasedAt || null,
            completedAt: after.completedAt || null,
          },
          escrowOutcomeLoggedAt: admin.firestore.FieldValue.serverTimestamp(),
        },
        { merge: true }
      );
    } catch (e) {
      console.error('[onJobEscrowReleasedUpdateTraining] ERROR:', e);
    }
...
async function createEscrowPaymentCore({ jobId, uid }) {
  if (!uid) {
    throw new functions.https.HttpsError('unauthenticated', 'Sign in required');
  }
  if (!jobId) {
  ...
  if (role !== 'customer') {
    throw new functions.https.HttpsError(
      'permission-denied',
      'Only customers can fund escrow'
    );
  }
  ...
  const job = jobSnap.data() || {};
  const requesterUid = (job.requesterUid || '').toString().trim();
  if (!requesterUid || requesterUid !== uid) {
    throw new functions.https.HttpsError(
      'permission-denied',
      'Only the job owner can fund escrow'
    );
  }

  const claimed = job.claimed === true;
  const contractorId = (job.claimedBy || '').toString().trim();
  if (!claimed || !contractorId) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Job must be accepted by a contractor before funding'
    );
  }

  const jobStatus = (job.status || '').toString().trim().toLowerCase();
  if (jobStatus === 'completed' || jobStatus === 'cancelled') {
    throw new functions.https.HttpsError(
      'failed-precondition',
      `Job cannot be funded in status: ${jobStatus}`
    );
  }

  const disputeStatus = (job.disputeStatus || '').toString().trim().toLowerCase();
  if (disputeStatus === 'open' || disputeStatus === 'under_review') {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Escrow cannot be funded while a dispute is open'
    );
  }

  const escrowStatus = (job.escrowStatus || '').toString().trim().toLowerCase();
  ...
  if (escrowStatus === 'funded' || escrowStatus === 'released') {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Escrow is already funded or released'
    );
  }

  const existingPi = (job.paymentIntentId || '').toString().trim();
  if (existingPi) {
    throw new functions.https.HttpsError(
      'already-exists',
      'Escrow payment already created for this job'
    );
  }

  const amountCents = dollarsToCents(job.price);
  if (!amountCents) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Job price is missing or invalid'
    );
  }
  ...
  if (!contractorStripeAccountId) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Contractor is not connected to payouts yet'
    );
  }

  const stripe = getStripeClient();
  const applicationFeeAmount = Math.round(amountCents * 0.075);
  ...
  await jobRef.set(
    {
      ...
      fundedAt: null,
      escrowStatusHistory: admin.firestore.FieldValue.arrayUnion([
        {
          status: 'funding',
          timestamp: admin.firestore.FieldValue.serverTimestamp(),
          updatedBy: uid,
          source: 'createEscrowPayment',
        },
      ]),
    },
    { merge: true }
...
async function releaseEscrowCore({ jobId, uid }) {
  if (!uid) {
    throw new functions.https.HttpsError('unauthenticated', 'Sign in required');
  }
  if (!jobId) {
  ...
  if (role !== 'customer') {
    throw new functions.https.HttpsError(
      'permission-denied',
      'Only customers can release escrow'
    );
  }
  ...
  if (!requesterUid || requesterUid !== uid) {
    throw new functions.https.HttpsError(
      'permission-denied',
      'Only the job owner can release escrow'
    );
  }

  // Require explicit approval before funds release.
  const jobStatus = (job.status || '').toString().trim().toLowerCase();
  const hasApproval = !!job.completionApproved || jobStatus === 'completion_approved';
  if (!hasApproval) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Completion must be approved before releasing escrow'
    );
  }
  ...
  if (disputeStatus === 'open' || disputeStatus === 'under_review') {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Escrow cannot be released while a dispute is open'
    );
  }

  // Backwards-compatible: older docs used status='funded'.
  const escrowStatus = (job.escrowStatus || '').toString().trim().toLowerCase();
  if (escrowStatus === 'frozen') {
    throw new functions.https.HttpsError(
      'failed-precondition',
  ...
  const isFunded = escrowStatus === 'funded' || jobStatus === 'funded';
  if (!isFunded) {
    throw new functions.https.HttpsError(
      'failed-precondition',
      'Escrow must be funded before it can be released'
    );
  }

  const paymentIntentId = (job.paymentIntentId || '').toString().trim();
  if (!paymentIntentId) {
    throw new functions.https.HttpsError(
      'failed-precondition',
...
// When a dispute is created, mark the job and freeze escrow (server as source of truth).
exports.onDisputeCreated = functions.firestore
  .document('disputes/{disputeId}')
  .onCreate(async (snap, context) => {
    const data = snap.data() || {};
    const jobId = (data.jobId || context.params.disputeId || '').toString().trim();
    if (!jobId) return;
    ...
    await db.runTransaction(async (tx) => {
      ...
      // Freeze escrow if it is mid-flight or funded but not yet released.
      if (escrowStatus === 'funding' || escrowStatus === 'funded') {
        updates.escrowStatus = 'frozen';
        updates.escrowStatusHistory = admin.firestore.FieldValue.arrayUnion([
          {
            status: 'frozen',
            timestamp: admin.firestore.FieldValue.serverTimestamp(),
            updatedBy: 'system',
            source: 'onDisputeCreated',
          },
        ]);
      }
...
exports.createEscrowPayment = functions
  .runWith({ secrets: [STRIPE_SECRET_KEY] })
  .https.onCall(
  async (data, context) => {
    const uid = context.auth?.uid;
    ...
    // Rate limiting: 20 escrow payments per day per user
    const rateLimit = await checkRateLimit(uid, 'createEscrowPayment', 20, 24 * 60 * 60 * 1000);
    if (!rateLimit.allowed) {
      const resetTime = new Date(rateLimit.resetTime).toISOString();
      throw new functions.https.HttpsError(
        'resource-exhausted',
        `Rate limit exceeded. You can create ${rateLimit.currentCalls} escrow payments per day. Try again after ${resetTime}.`
      );
    }
    
    return await createEscrowPaymentCore({ jobId, uid });
  }
);

// Desktop-safe endpoint (callable isn't implemented on Windows).
// Call with: Authorization: Bearer <Firebase ID token>
exports.createEscrowPaymentHttp = functions
  .runWith({ secrets: [STRIPE_SECRET_KEY] })
  .https.onRequest(async (req, res) => {
  if (req.method === 'OPTIONS') {
    res.set('Access-Control-Allow-Origin', '*');
  ...
  try {
    ...
    const jobId = (req.body?.jobId || '').toString().trim();
    const result = await createEscrowPaymentCore({ jobId, uid });
    res.json(result);
  } catch (err) {
    if (err && err.code && err.message) {
      const code = err.code;
...
exports.releaseEscrow = functions
  .runWith({ secrets: [STRIPE_SECRET_KEY] })
  .https.onCall(
  async (data, context) => {
    const uid = context.auth?.uid;
    ...
    // Rate limiting: 30 escrow releases per day per user
    const rateLimit = await checkRateLimit(uid, 'releaseEscrow', 30, 24 * 60 * 60 * 1000);
    if (!rateLimit.allowed) {
      const resetTime = new Date(rateLimit.resetTime).toISOString();
      throw new functions.https.HttpsError(
        'resource-exhausted',
    ...
    return await releaseEscrowCore({ jobId, uid });
  }
);

// Desktop-safe endpoint (callable isn't implemented on Windows).
// Call with: Authorization: Bearer <Firebase ID token>
exports.releaseEscrowHttp = functions
  .runWith({ secrets: [STRIPE_SECRET_KEY] })
  .https.onRequest(async (req, res) => {
  if (req.method === 'OPTIONS') {
    res.set('Access-Control-Allow-Origin', '*');
  ...
  try {
    ...
    const jobId = (req.body?.jobId || '').toString().trim();
    const result = await releaseEscrowCore({ jobId, uid });
    res.json(result);
  } catch (err) {
    if (err && err.code && err.message) {
      const code = err.code;
...
async function createCheckoutSessionCore({ jobId, uid }) {
  ...
  // Lead unlock price should come from claimCost (not the job escrow price).
...
exports.stripeWebhook = functions
  ...
  // Escrow funding: for manual-capture PaymentIntents, Stripe emits an event
  // when the intent becomes capturable (status: requires_capture).
  if (event.type === 'payment_intent.amount_capturable_updated') {
    const pi = event.data.object;
    const jobId = (pi.metadata?.jobId || '').toString().trim();
    if (jobId && pi.status === 'requires_capture') {
      const db = admin.firestore();
      const jobRef = db.collection('job_requests').doc(jobId);
      const snap = await jobRef.get();
      if (snap.exists) {
        const current = snap.data() || {};
        const expectedPi = (current.paymentIntentId || '').toString().trim();
        if (!expectedPi || expectedPi === pi.id) {
          await jobRef.set(
            {
  ...
  .onUpdate(async (change, context) => {
    try {
      ...
      if (newStatus === 'in_progress') {
        ...
        // Job completed means escrow has been released (server-owned transition).
        recipientId = contractorId;
