...
class PriceGuaranteeAdminTab extends StatefulWidget {
  ...
  @override
  State<PriceGuaranteeAdminTab> createState() =>
      _PriceGuaranteeAdminTabState();
}
...
class _PriceGuaranteeAdminTabState extends State<PriceGuaranteeAdminTab> {
                          ...
                          if (widget.canWrite) ...[
                            IconButton(
                              icon: const Icon(Icons.edit,
                                  size: 18, color: AdminColors.accent2),
                              onPressed: () =>
                                  _showRuleDialog(context, r),
                            ),
                            IconButton(
    ...
    return _violations.isEmpty
        ...
        : ListView.separated(
            ...
            itemBuilder: (context, i) {
              ...
              final refund =
                  (v['refundAmount'] as num?)?.toDouble() ?? 0;
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('Job: ${jobId.toString().substring(0, jobId.toString().length.clamp(0, 12))}…',
                              style: const TextStyle(
  ...
  void _showRuleDialog(BuildContext context, Map<String, dynamic>? rule) {
    final serviceCtrl =
        TextEditingController(text: rule?['service'] as String? ?? '');
          ...
          FilledButton(
            onPressed: () async {
              final data = {
                ...
                'maxGuaranteePrice':
                    double.tryParse(maxCtrl.text.trim()) ?? 0,
