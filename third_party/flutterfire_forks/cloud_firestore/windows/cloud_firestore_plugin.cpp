...
// A private message used to run queued tasks on the platform/UI thread.
// WM_APP range is reserved for application-defined messages.
constexpr UINT kCloudFirestorePlatformTaskMessage = WM_APP + 0x3F10;

HWND g_platform_hwnd = nullptr;
int g_platform_wndproc_delegate_id = 0;

std::mutex g_platform_tasks_mutex;
std::queue<std::function<void()>> g_platform_tasks;

void DrainPlatformTasks() {
  for (;;) {
    std::function<void()> task;
    {
      std::lock_guard<std::mutex> lock(g_platform_tasks_mutex);
      if (g_platform_tasks.empty()) {
        break;
      }
      task = std::move(g_platform_tasks.front());
      g_platform_tasks.pop();
    }
    if (task) {
      task();
    }
...
void PostToPlatformThread(std::function<void()> task) {
  // If we don't yet have a window handle (should be rare), run inline.
  if (g_platform_hwnd == nullptr) {
    if (task) {
      task();
    }
    return;
  ...
  {
    std::lock_guard<std::mutex> lock(g_platform_tasks_mutex);
    g_platform_tasks.push(std::move(task));
  }

  // Trigger the task drain on the platform thread.
  PostMessage(g_platform_hwnd, kCloudFirestorePlatformTaskMessage, 0, 0);
}
...
std::optional<LRESULT> HandleTopLevelWindowProc(HWND hwnd,
                                                ...
                                                WPARAM wparam,
                                                LPARAM lparam) {
  if (message != kCloudFirestorePlatformTaskMessage) {
    return std::nullopt;
  }
  if (hwnd != g_platform_hwnd) {
    return std::nullopt;
  }
  DrainPlatformTasks();
  return 0;
...
void CloudFirestorePlugin::RegisterWithRegistrar(
  ...
  // Capture the top-level window handle so we can marshal snapshot callbacks
  // to the platform thread (required for platform channel messages).
  if (auto* view = registrar->GetView()) {
    g_platform_hwnd = view->GetNativeWindow();
  }
  if (g_platform_hwnd != nullptr && g_platform_wndproc_delegate_id == 0) {
    g_platform_wndproc_delegate_id =
        registrar->RegisterTopLevelWindowProcDelegate(HandleTopLevelWindowProc);
  }
...
class QuerySnapshotStreamHandler
 ...
 public:
  ...
  OnListenInternal(
    ...
    events_ = std::shared_ptr<flutter::EventSink<flutter::EncodableValue>>(
      std::move(events));
    std::weak_ptr<flutter::EventSink<flutter::EncodableValue>> weak_events =
      events_;

    listener_ = query_->AddSnapshotListener(
        metadataChanges,
        [weak_events, serverTimestampBehavior = serverTimestampBehavior_,
         metadataChanges](const firebase::firestore::QuerySnapshot& snapshot,
          ...
          if (error == firebase::firestore::kErrorOk) {
            ...
            PostToPlatformThread(
                [weak_events, toListResult = std::move(toListResult)]() mutable {
                  if (auto events = weak_events.lock()) {
            ...
            PostToPlatformThread([weak_events, errorMessage, details]() mutable {
              if (auto events = weak_events.lock()) {
                events->Error("firebase_firestore", errorMessage, details);
                events->EndOfStream();
              }
            });
          }
 ...
 private:
  ...
  std::shared_ptr<flutter::EventSink<flutter::EncodableValue>> events_;
          ...
          if (error == firebase::firestore::kErrorOk) {
            auto parsed =
                ParseDocumentSnapshot(snapshot, serverTimestampBehavior)
                    .ToEncodableList();
            PostToPlatformThread(
                [weak_events, parsed = std::move(parsed)]() mutable {
                  if (auto events = weak_events.lock()) {
                    events->Success(parsed);
                  }
                });
          } else {
            EncodableMap details;
