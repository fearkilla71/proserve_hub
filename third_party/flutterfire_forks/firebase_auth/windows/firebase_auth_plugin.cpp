...
constexpr UINT kFirebaseAuthPlatformTaskMessage = WM_APP + 0x3F11;
...
void PostToPlatformThread(std::function<void()> task) {
  ...
  PostMessage(g_platform_hwnd, kFirebaseAuthPlatformTaskMessage, 0, 0);
}
...
std::optional<LRESULT> HandleTopLevelWindowProc(HWND hwnd,
                                                ...
                                                WPARAM wparam,
                                                LPARAM lparam) {
  if (message != kFirebaseAuthPlatformTaskMessage) {
    return std::nullopt;
  }
...
void FirebaseAuthPlugin::RegisterWithRegistrar(
  ...
  // Capture the top-level window handle so we can marshal stream events to the
  // platform thread (required for platform channel messages).
...
class FlutterIdTokenListener : public firebase::auth::IdTokenListener {
 public:
  ...
  void OnIdTokenChanged(Auth* auth) override {
    ...
    auto weak_sink = std::weak_ptr<flutter::EventSink<flutter::EncodableValue>>(
        event_sink_);
    if (auto sink = weak_sink.lock()) {
      if (user.is_valid()) {
        ...
        auto payload = EncodableValue(
            EncodableMap{{EncodableValue("user"), userDetailsList}});
        PostToPlatformThread(
            [weak_sink, payload = std::move(payload)]() mutable {
              if (auto s = weak_sink.lock()) {
                s->Success(payload);
              }
            });
      } else {
        auto payload = EncodableValue(EncodableMap{
