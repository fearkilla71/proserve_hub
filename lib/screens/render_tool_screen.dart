...
class _RenderToolScreenState extends State<RenderToolScreen> {
  ...
  Color _aiWallColor = const Color(0xFF2E7DFF);
  Color _aiCabinetColor = const Color(0xFF00BFA5);
  bool _aiBusy = false;
  ...
  Future<void> _runAiRender() async {
    if (_aiBusy) return;
    final imgBytes = _originalBytes;
    if (imgBytes == null || imgBytes.isEmpty) return;
    ...
    try {
      // Keep payload small enough for callable functions.
      ...
      final callable = FirebaseFunctions.instance.httpsCallable('aiRenderRecolor');
      final resp = await callable.call(<String, dynamic>{
        'imageBase64': base64Encode(compressed),
        'wallColor': _toHexRgb(_aiWallColor),
        'cabinetColor': _toHexRgb(_aiCabinetColor),
      });
    ...
    setState(() {
      if (wall) {
        _aiWallColor = selected;
      } else {
        _aiCabinetColor = selected;
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            tooltip: 'AI render (OpenAI)',
            onPressed: img == null || _aiBusy ? null : _runAiRender,
            icon: _aiBusy
                ? const SizedBox(
                    width: 22,
                    height: 22,
          ...
          IconButton(
            ...
            onPressed: img == null || _exporting ? null : _exportAndShare,
            icon: _exporting
                ? const SizedBox(
                    width: 22,
                    height: 22,
                  ...
                  onToggleEraser: () =>
                      setState(() => _eraserMode = !_eraserMode),
                            ...
                            if (_mode == _RenderMode.ai)
                              _AiControlsBar(
                                wallColor: _aiWallColor,
                                cabinetColor: _aiCabinetColor,
                                hasResult: aiImg != null,
                                busy: _aiBusy,
                                onPickWallColor: () => _pickAiColor(wall: true),
                                onPickCabinetColor: () => _pickAiColor(wall: false),
                                onGenerate: _runAiRender,
                                onClearResult: () {
                                  setState(() {
                                    _aiDecoded?.dispose();
                                    _aiDecoded = null;
                      ...
                      return Center(
                        child: GestureDetector(
                          behavior: HitTestBehavior.opaque,
                                  ...
                                  final displayImage =
                                      (_mode == _RenderMode.ai && !_showOriginal && aiImg != null)
                                          ? aiImg
                                          : img;
                                  ...
                                  final preview = RepaintBoundary(
                                    ...
                                    child: CustomPaint(
                                      ...
                                      foregroundPainter: _mode == _RenderMode.manual
                                          ? _RenderPainter(
                                              image: img,
                                              mapping: mapping,
                                              strokes: List<_Stroke>.unmodifiable(_strokes),
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
            ...
            class _AiControlsBar extends StatelessWidget {
                    ...
                    child: Column(
                      children: [
                          ...
                          children: [
                            Expanded(
                              child: FilledButton.icon(
                                ...
                                label: Text(busy ? 'Rendering…' : 'AI Render'),
                              ),
                            ),
                            const SizedBox(width: 10),
                            IconButton(
                              tooltip: 'Clear AI result',
                              onPressed: hasResult && !busy ? onClearResult : null,
                        ...
                        Align(
                          ...
                          child: Text(
                            'Tip: AI render is best with a well-lit, straight-on photo of the room.',
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Take a picture and paint over walls to preview a new color.\n(Manual mask — no AI yet.)',
...
class _Stroke {
  ...
  final double width;
  final bool eraser;
  final List<Offset> points; // normalized [0..1] within image rect
...
class _CanvasMapping {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
              ...
              children: [
                ...
                Expanded(
                  child: _ColorPill(
                    ...
                    onTap:
                        cabinetsEnabled && !busy ? onPickCabinetColor : null,
                  ),
                ),
              ],
            ),
      ...
      final path = Path();
      path.moveTo(
        mapping.toCanvas(stroke.points.first).dx,
        mapping.toCanvas(stroke.points.first).dy,
      );
